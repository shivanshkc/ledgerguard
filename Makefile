...
application_name           = ledgerguard
application_binary_name    = main
application_image_name     = ledgerguard
application_container_name = ledgerguard-1
...
container:
	...
	@docker run \
		...
		--net host \
		--volume $(PWD)/configs.yaml:/etc/$(application_name)/configs.yaml \
